...
describe('PrismaPostRepository (integration)', () => {
  it('list({ nightId }) filters by nightId and orders desc by postedAt', async () => {
    ...
    await repo.save(newPost('p1', alice, new Date('2026-05-25T13:00:00Z')))
    await repo.save(newPost('p2', alice, new Date('2026-05-25T13:30:00Z')))
    await repo.save(newPost('p3', bob, new Date('2026-05-23T13:00:00Z')))
  ...
  it('list({ authorId }) returns the author posts across nights, desc by postedAt', async () => {
    const repo = createPrismaPostRepository(prisma)
    await repo.save(newPost('p1', alice, new Date('2026-05-23T13:00:00Z')))
    await repo.save(newPost('p2', alice, new Date('2026-05-25T13:00:00Z')))
    await repo.save(newPost('p3', bob, new Date('2026-05-25T13:30:00Z')))
  ...
  it('save is upsert — second save with deletedAt updates the row', async () => {
    const repo = createPrismaPostRepository(prisma)
    const p = newPost('p1', alice, new Date('2026-05-25T13:00:00Z'))
    await repo.save(p)
    await repo.save({ ...p, deletedAt: new Date('2026-05-25T14:00:00Z') })
